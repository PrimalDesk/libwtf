...
typedef enum {
    ...
    WTF_SETTING_WT_MAX_SESSIONS = 0x14e9cd29
} wtf_h3_setting_t;

// This ID is larger than INT_MAX and therefore cannot be represented by a C
// enum on MSVC without sign extension when it is promoted to uint64_t.
#define WTF_SETTING_WEBTRANSPORT_MAX_SESSIONS_DRAFT07 UINT64_C(0xc671706a)
